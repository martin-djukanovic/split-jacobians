/* The base field is not of characteristic 2 or 3 and w denotes a primitive third root of unity. A plane model for C(a,b) whose Jacobian
   is obtained by gluing
   	E1: x^3+y^3+z^3+3*a*x*y*z=0
	E2: x^3+y^3+z^3+3*b*x*y*z=0
   along the 3-torsion by taking the quotient (E1 x E2)/G where G is the isomorphism E1[3]-->E2[3] given by
   	[-1:0:1] |--> [-1:0:1]
	[-w:1:0] |--> [-w^2:1:0]
  is obtained in curve_C_in_P2_(derivation).m. Here we give explicit isomorphisms
   C(a,b) --> C(a*w,b*w^2)
   C(a,b) --> C(a*w^2,b*w)
...
iso3:=map<C->C3|[
   -((3*a^3*b - a^3 - b^3 + 3*a*b^2 - 3*a^2 - 3*a*b - 3*b + 1)*x + 3*a^2*b^2 + a^3 + b^3 - 3*a*b + 2)/((3*a^2*b^2 + a^3 + b^3 - 3*a*b + 2)*x + 3*a*b^3 - a^3 - b^3 + 3*a^2*b - 3*b^2 - 3*a*b - 3*a + 1), 
   729*(1 + 2*w)*(a*b - 1)^3*(a^2 + b^2 - a*b + a + b + 1)^3/(a + 1)^5/(b + 1)^5 * y/((3*a^2*b^2 + a^3 + b^3 - 3*a*b + 2)*x + 3*a*b^3 - a^3 - b^3 + 3*a^2*b - 3*b^2 - 3*a*b - 3*a + 1)^3
]>;
